...
class CarController:
    ...
    def calculate_speed(self):
        self._car_speed = min(abs(int(abs(self._error) * (self.speed["max"] /4))) + self.speed["min"], self.speed["max"])
    ...
    def get_line_state(self):
        ...
        if self.control_variables["line_color"] == "White":
            return line_state
    ...
    def calculate_error(self):
        ...
        self.pid_values["prev_error"] = self.pid_values["error"]
        ...
        try:
            self.pid_values["error"] = (4*RR + 2*RM + 0 + -2*LM + -4*LL) / (RR + RM + MM + LM + LL)
        except ZeroDivisionError:
            print("not on line")
        if abs(self.pid_values["error"]) < 4:
            self.pid_values["PV"] += -.0001 * self.pid_values["error"]

        self.pid_values["PV"] = max(self.pid_values["PV"],-10)
        self.pid_values["PV"] = min(self.pid_values["PV"],10)
            
        return self.pid_values["error"]

    def auton_control_update(self):
        while True:
            if self.control_variables["control_type"] == 1:
                _error = self.calculate_error()
                correction = self.pid_values["p"] * _error + self.pid_values["i"] * self.pid_values["PV"] + self.pid_values["d"] * (self.pid_values["error"] -self.pid_values["prev_error"])
                self._motor_driver.Turn(correction)
                self._motor_driver.Drive(self.calculate_speed())

    def car_auto_stop(self):
        while True:
            if self.control_variables["control_type"] == 0: 
                if time.time() - self.last_times["last_velo_time"] > .05 and not self.speed["current_speed"] == 0 :
                    self.stop()
                if time.time() - self.last_times["last_steer_time"] > .05 and not self.control_variables["straight"]:
                    self.control_variables["straight"] = 1
